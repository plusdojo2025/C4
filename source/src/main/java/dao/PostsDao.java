...
public class PostsDao {
	...
	public List<PostsDto> selectByUserId(int userId) {
		...
		String sql = "SELECT p.*, u.name AS user_name, "
				+ "(SELECT COUNT(*) FROM reactions r WHERE r.post_id = p.post_id) AS likeCount " + "FROM posts p "
				+ "JOIN users u ON p.user_id = u.user_id " + "WHERE p.user_id = ? " + "ORDER BY p.created_at DESC";
	...
	public List<PostsDto> select(PostsDto condition) {
		List<PostsDto> list = new ArrayList<>();
		StringBuilder sql = new StringBuilder("SELECT p.*, u.name AS user_name " + "FROM posts p "
